...
### `APT2000: Не удается найти указанный файл ... assets`

**Причина:** путь к проекту содержит **кириллицу** или другие не-ASCII символы (например `D:\Проект\...`). Инструмент Android `aapt2` на Windows с такими путями не работает.

**Решение (рекомендуется):** перенесите проект в путь только с латиницей:

```
C:\Projects\APK_Webbrowser
```

**Обход:** `build.bat` / `WebViewer\build.ps1` автоматически попробуют собрать проект через временную ASCII-папку и скопируют APK обратно.
